...
public class TapePlayerCarControls : MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    private float _accelerateStrength = 20;
...
void FixedUpdate()
{
    var accelerate = _input.Player.Accelerate.ReadValue<float>();
    var reverse = _input.Player.Reverse.ReadValue<float>();
    ...
    _rb.AddTorque(transform.forward * (accelerate - reverse) * _accelerateStrength, ForceMode.Acceleration);
}
}
